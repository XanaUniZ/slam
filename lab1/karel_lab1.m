...
world.true_point_locations = [0.5:1:100]';
...
% Build the H matrix
N = length(measurements.z_k)+1;
all_ids = [measurements.ids_f, measurements.ids_n];
% Initialize the matrix with zeros
H_k = zeros(length(measurements.z_k), length(map.hat_x));
...
% estimated_feat_R0 = map.hat_x(1) + measurements.z_n;
% map.hat_x = [map.hat_x; estimated_feat_R0];

% Update coveriance matrix
map.hat_P = J1*map.hat_P*J1.' + J2*measurements.R_n*J2.';

% global sensor;
% new_cov = (sensor.factor_z * measurements.z_n).^2;  
% % new_cov = diag(measurements.R_n);
% map.hat_P = blkdiag(map.hat_P, diag(new_cov));
% 
% Add the new features map.true_x
% true_feat_R0 = map.true_x(1) + measurements.z_pos_n;
map.true_x = J1*map.true_x + J2*measurements.z_pos_n;
